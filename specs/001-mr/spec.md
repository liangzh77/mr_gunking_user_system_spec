...
- **FR-055**: 系统必须实施请求频率限制（单运营商每分钟最多10次授权请求，使用PostgreSQL实现计数器）
...
- **NFR-015**: 所有API遵循OpenAPI 3.0规范，契约测试覆盖率100%
...
- **支付平台回调时效**: 假设微信/支付宝支付回调正常在30秒内到达，超时5分钟后系统主动查询支付平台确认结果（通过定时任务每5分钟执行）
...
- **系统容量目标**: 授权API峰值吞吐量20 req/s，单实例部署，PostgreSQL默认连接池配置即可满足需求
