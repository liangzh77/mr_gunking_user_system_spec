...
# 查看特定服务日志
docker-compose -f docker-compose.yml --env-file .env.production logs -f backend
docker-compose -f docker-compose.yml --env-file .env.production logs -f frontend
docker-compose -f docker-compose.yml --env-file .env.production logs -f postgres
```
...
```bash
# 查看错误日志
docker-compose -f docker-compose.yml --env-file .env.production logs backend
...
# 重新构建前端
docker-compose -f docker-compose.yml --env-file .env.production up -d --build frontend
```
...
# 停止所有服务
docker-compose -f docker-compose.yml --env-file .env.production stop

# 启动所有服务
docker-compose -f docker-compose.yml --env-file .env.production start
...
# 查看特定时间的日志
docker-compose -f docker-compose.yml --env-file .env.production logs --since 2025-10-28T10:00:00
```
